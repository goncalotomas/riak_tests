-module(http_bucket_types_SUITE).
...
-export([
    default_type_get_put/1
    ,default_type_list_keys/1
    ,default_type_list_buckets/1
    ,default_type_delete/1
    ,custom_type_get_put/1
    ,custom_type_list_keys/1
    ,custom_type_list_buckets/1
    ,utf8_type_get_put/1
    ,utf8_type_list_keys/1
    ,utf8_type_list_buckets/1
    ,bucket_and_bucket_type_properties/1
    ,secondary_indexes/1
]).
...
init_per_suite(Config) ->
    rt_ct_util:start_node('http_bucket_types@127.0.0.1'),
    rt_ct_util:setup(),
    ...
    Node = hd(Nodes),
    [{node, Node}, {nodes, Nodes},  {unicode_type_bin, UnicodeTypeBin}, {unicode_bucket_bin, UnicodeBucketBin}, {ucbbin, UCBBin} | Config].
...
all() ->
    [
        default_type_get_put
        ,default_type_list_keys
        ,default_type_list_buckets
        ,default_type_delete
        ,custom_type_get_put
        ,custom_type_list_keys
        ,custom_type_list_buckets
        ,utf8_type_get_put
        ,utf8_type_list_keys
        ,utf8_type_list_buckets
        ,bucket_and_bucket_type_properties
        ,secondary_indexes
    ].

default_type_get_put(Config) ->
...
default_type_list_buckets(Config) ->
    Node = ?config(node, Config),
    RHC = rt:httpc(Node),
    %% list buckets
...
custom_type_list_buckets(Config) ->
    Node = ?config(node, Config),
    RHC = rt:httpc(Node),
...
utf8_type_get_put(Config) ->
    Nodes = ?config(nodes, Config),
    Node = ?config(node, Config),
    RHC = rt:httpc(Node),
    lager:info("UTF-8 type get/put test"),
    UnicodeTypeBin = ?config(unicode_type_bin, Config),
    UCBBin = ?config(ucbbin, Config),
...
bucket_and_bucket_type_properties(Config) ->
    Nodes = ?config(nodes, Config),
    Node = ?config(node, Config),
    RHC = rt:httpc(Node),
