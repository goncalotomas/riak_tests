...
-export([
    relative_message_redundancy_check/1
]).
...
init_per_suite(Config) ->
    ...
    random:seed(Seed),
    [{num_nodes, 5}, {super_rounds, 1}, {num_rounds, 5}, {stable_rounds, 1}  | Config].
...
all() ->
    [
        relative_message_redundancy_check
    ].

relative_message_redundancy_check(Config) ->
    NumNodes = ?config(num_nodes, Config),
    SuperRounds = ?config(super_rounds, Config),
    NumRounds = ?config(num_rounds, Config),
    StableRounds = ?config(stable_rounds, Config),
    run(NumNodes, SuperRounds, NumRounds, StableRounds),
    ok.

run(NumNodes, SuperRounds, NumRounds, StableRounds) when is_integer(NumNodes) ->
    run([NumNodes], SuperRounds, NumRounds, StableRounds);
